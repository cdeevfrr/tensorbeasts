...
- a function to take a dungeonState and a caller monster and to execute the skill (returning the modified dungeon state). 
   - Note that the JSON you get for self (the current skill being executed) may be of an OLD type for that skill. If someone got a beast in v1 with a certain skill, they may use that same beast's skill in v2. 
